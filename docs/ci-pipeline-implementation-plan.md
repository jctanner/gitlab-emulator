...
- minimal `needs` and common rules/ref filters are implemented: jobs can unlock
  ...
  operators, `exists`, commit-local `changes`, `when: never`, and persisted
...
- cache key prefix/files parsing and emulator cache fallback-key lookup are
  implemented for API-level cache coverage; official runner validation remains
  on GitLab Runner's S3 cache adapter backed by MinIO
...
- deeper `rules` / `only` / `except` edge cases
...
- support richer cache options and edge cases; GitLab Runner supports S3/GCS/Azure
