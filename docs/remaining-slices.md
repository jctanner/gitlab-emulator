...
workflow variables, `allow_failure`, grouped boolean `rules:if` expressions,
regex non-match operators, null/empty variable comparisons, `exists`/`changes`
path-object rules, variable-expanded rule path patterns, mapping-form
`only`/`except`, cache list/file-derived keys, cache variable expansion, richer
