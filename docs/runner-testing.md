...
grouped `rules:if` boolean expressions, `exists`/`changes` path-object rule
...
intentionally temporary. Cache metadata,
variable-expanded cache keys/policies/fallback keys, and archive endpoints are
