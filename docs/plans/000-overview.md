...
- Minimal CI support exists for stages, image, structured image metadata
  ...
  cases including same-pipeline `needs:parallel:true`,
  same-pipeline `needs:parallel:matrix`, artifact-oriented
  `needs:project` and `needs:pipeline:job`,
  ...
  `rules:changes:compare_to`, variable-expanded rule path patterns,
  ...
  expansion, artifact report payloads, job `dependencies` artifact selection
...
- Minimal pipeline trigger token APIs, push-created pipelines, and pipeline
  schedule APIs exist. Successful Git Smart HTTP/SSH branch and tag pushes and
  repository file API commits create `source=push` pipelines when
  `.gitlab-ci.yml` is present, trigger tokens can create `source=trigger`
  ...
  pipeline bridges API. Project bridge jobs accept validated `trigger:forward`
  metadata.
...
- GitLab global search supports projects, users, issues, merge requests,
  milestones, indexed blobs/code, and indexed commits with readable-project
  filtering for project-backed result types.
