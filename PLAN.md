...
- Minimal CI support exists for stages, image, variables, before_script, script,
  ...
  `exists`/`changes` path-object rules, rule-level variables,
  `allow_failure` scheduling/status behavior, runner tags, cache metadata with
...
- deeper `rules` behavior beyond the current MVP expression, exists, changes,
  exists/changes path-object, never, manual, rule-variable, and
